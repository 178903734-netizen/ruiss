...
- core/keys.rs — 键位映射：Ctrl ↔ Command（Mac 上 Command 才是 Ctrl 的位）；Win 键 → Mac Command；Shift/Alt 不动。第一版就这三条规则（2026-08-08 补齐导航键与标点符号映射：Delete/Home/End/PageUp/PageDown/Insert/CapsLock + 逗号句号等，解决 Win 按键在 Mac 端失效）。
- platform/ — 事件捕获与注入，每端约 200 行。Windows: SetWindowsHookEx 捕获 + SendInput 注入；Mac: CGEventTap 捕获 + CGEventPost 注入（2026-08-08：注入器复用同一 CGEventSource，修复双击/滑动选中失效——macOS 按 event source 追踪点击计数）。
